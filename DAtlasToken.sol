...
contract IMigrationContract {
    function migrate(address addr, uint256 DA) returns (bool success);
}
...
contract DAtlasToken is StandardToken, SafeMath {

    // metadata
    string  public constant name = "DAtlas Token";
    string  public constant symbol = "DA";
    uint256 public constant decimals = 18;
    string  public version = "1.0";

    // contracts
    address public ethFundDeposit;          // deposit address for ETH for DAtlas Team.
    address public newContractAddr;         // the new contract for DAtlas token updates;
    ...
    uint256 public tokenExchangeRate = 625;             // 625 DA tokens per 1 ETH
    ...
    /// @dev sends ETH to DAtlas team
    function transferETH() isOwner external {
        if (this.balance == 0) throw;
        if (!ethFundDeposit.send(this.balance)) throw;
    }

    /// @dev allocates DA tokens to pre-sell address.
